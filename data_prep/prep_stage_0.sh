...
# Get scan lengths for all available scans
    echo $sub >> $STAGE_0_OUT/subs_and_lengths.txt > /dev/null 2>&1
    find $BIDS_PATH/$sub/ses-baselineYear1Arm1/func/ -type f -name "*task-rest_run*[0-9][0-9]_bold.nii.gz" | sort | xargs -L 1 fslnvols | tee -a $STAGE_0_OUT/subs_and_lengths.txt | tee $PRE_CENSOR_LENGTHS/${sub}.txt > /dev/null 2>&1
