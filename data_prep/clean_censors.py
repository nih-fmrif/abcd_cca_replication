...
# Argument: list of subjects in format sub-NDARINVxxxxxxx
...
# format of each line in text file is sub-NDARINVxxxxxxx on each line (need to drop the sub- prefix)
...
for sub in subs:
    ...
    sub2=sub.split('sub-')[1]
    fp_censor=os.path,join(cwd,"censoring_data/"+sub2+"_censor.txt")
    fp_scan_class=os.path.join(cwd,"censoring_data/"+sub2+"_scans_classified.txt")
    fp_lens=os.path.join(cwd,"censoring_data/"+sub2+"_scan_lengths.txt")
    ...
    if (os.path.exists(fp_censor) & os.path.exists(fp_scan_class) & os.path.exists(fp_lens)):
        ...
        if len(scans) == len(scan_lengths):
            ...
            with open(os.path.join(cwd,"censoring_data/"+sub2+"_censor_subset.txt"), "w") as output:
...
if len(failed_subs) > 0:
    print("Subjects whose censor files couldn't be edited:\n")
    print(failed_subs)
else:
