...
% mcc2 -m hcp_cca_batch.m -d compiled_scripts/ -nojvm -a ../dependencies/palm-alpha116/palm_inormal.m
...
function hcp_cca_batch(start_idx_in, num_perms_in, N_dim_in, abcd_cca_dir, hcp_cca_dir, n_subs_in)
    if nargin<6
        sprintf("ERROR, not enough arguments.")
        sprintf("Example: hcp_cca_batch(1, 2000, 100, '/data/ABCD_MBDU/goyaln2/abcd_cca_replication/', '/data/ABCD_MBDU/goyaln2/abcd_cca_replication/hcp_cca_testing/', 461)")
        return
	end
    ...
    s1  =   sprintf('%s/data/%d/S1.txt', hcp_cca_dir, n_subs);
    % Matrix S5 (post-PCA SM matrix)
    s5  =   sprintf('%s/data/%d/S5.txt', hcp_cca_dir, n_subs);
    % Matrix N0 (raw connectome data)
    n0  =   sprintf('%s/data/%d/N0.txt', hcp_cca_dir, n_subs);
    % Matrix N5 (post-PCA connectome matrix)
    n5  =   sprintf('%s/data/%d/N5.txt', hcp_cca_dir, n_subs);
    ...
    % Permutation matrix
    pset=sprintf('%s/data/%d/Pset.txt', hcp_cca_dir, n_subs);
    Pset=load(pset);
    ...
    save(sprintf('%s/data/%d/permutations/permutations_%d.mat', hcp_cca_dir, n_subs, start_idx), 's');
